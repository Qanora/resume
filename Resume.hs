...
basicInfo = paragraph
  ...
  , pure $ "\\homepage{https://ice1000.org}" ++ period
...
sec = tex "section"
url s = pure $ "\\url{" ++ s ++ "}"
github s = url $ "https://github.com/" ++ s
bold s = pure $ "\\textbf{" ++ s ++ "}"
...
resume = paragraph
  ...
  -- education
  , let gpa = "GPA 3.20/4.00"
    in section "教育经历" "Education"
    [ datedSection (date "2018" "08" ~~ present) $ paragraph
      [ en "\\textbf{The Pennsylvania State University (PSU)}, PA, US"
      , cn "\\textbf{宾夕法尼亚州州立大学 (PSU)}, 美国"
      ]
  ...
  , section "工作经历" "Work Experience"
    ...
    , paragraph
      ...
      , let grpcio   = "\\href{https://docs.rs/crate/grpcio} {grpcio}"
        ...
        , cn "\\item 学到了很多 Rust 编程、分布式和数据库相关的知识"
        , elab "\\item Learned a lot about Rust programming, distributed system, and databases."
        ]
      ]

    , paragraph
      [ datedSection (date "2020" "01" ~~ present) $ paragraph
        [ cn "\\textbf{JetBrains s.r.o.}, 远程"
        , en "\\textbf{JetBrains s.r.o.}, Remote"
        ]
      , paragraph
  ...
  , section "个人项目" "Personal Projects"
    ...
    , paragraph
      ...
      , cn "实验型依赖类型编程语言，支持 Row-polymorphism"
      , en "Experimental dependently-typed programming language with row-polymorphism"
  ...
  , section "技能" "Skills"
    [ let somehow = "Dart C C++ F\\# F$\\star$ Idris Perl"
    ...
    , cn "有使用 YouTrack、Jira、GitHub、BitBucket、Coding.net、Tower 等团队协作工具的经验"
    , en "can adapt to any editors/OSs, usually use JetBrains IDEs and Emacs under Ubuntu"
    , en "and Windows Subsystem of Linux,"
    , en "have experience with team collaboration tools like YouTrack,"
    , en "Jira, GitHub, BitBucket, Coding.net, Tower."
    ] ]
  ...
  , section "其他" "Miscellaneous"
    ...
    , cn $ "3000+ 声誉，同时也在" ++ stackex ++ "{其他 StackExchange 子站}活跃"
    , en $ "3000+ reputations, also active on " ++ stackex ++ "{other StackExchange sites}"
